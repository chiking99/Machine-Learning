...
def exercises_sonnet():
    ...
    lm = LanguageModel(ngram_size, dataset.tokenizer, Dimension)
    # Train LM
    ...
    pred_word=dataset.tokenizer.index_word[pred_index]
    prob_score=logits[0][pred_index]
    print("Predict the next word:", pred_word,"\nProbabiliy score", prob_score)
...
def exercises_toy():
    ...
    pred_word=dataset.tokenizer.index_word[pred_index]
    prob_score=logits[0][pred_index]
    crook_index=dataset.tokenizer.word_index['crook']
    crook_prob_score=logits[0][crook_index]
    print("Predict the next word:", pred_word,"\n",pred_word,"Probabiliy score", prob_score,"\n crook Probability score",crook_prob_score)
    ...
    def consinesimilarity(x,y):
        con_sim=np.dot(x,y)/(norm(x)*norm(y))
        return con_sim
    print("Consine similarity for thief and crook: ",consinesimilarity(thief,crook),"\nConsine similarity for thief and cop: ",consinesimilarity(thief,cop))
